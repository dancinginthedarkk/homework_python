...
while True:
    if menu_level == 1:
        ...
        if answer == '2':
            ...
            print("Вы выбрали БД :" + way)
        elif answer == '0':
            break
        else:
            print("Невозможно обработать запрос")
        ...
        if answer == '0':
            ...
            fr = input("FROM: ")
            where = input("WHERE: ")
            t_1 = "SELECT " + sel + " FROM " + fr + " WHERE " + where + ";"
            if cursor:
                cursor.execute(t_1)
            ...
            values = input("VALUES: ")
            name = input("NAME: ")
            t_2 = "INSERT INTO " + insert + " VALUES (\""+ values +"\", \""+ name +"\");"
            if cursor:
                cursor.execute(t_2)
                print(cursor.fetchall())
                connection.commit()
            ...
            where_2 = input("WHERE: ")
            t_3 = "DELETE FROM " + delete + " WHERE " + where_2 + ";"
            if cursor:
                cursor.execute(t_3)
                print(cursor.fetchall())
                connection.commit()
            ...
            set_ = input("SET: ")
            where_3 = input("WHERE: ")
            t_4 = "UPDATE " + update + " SET " + set_ +" WHERE " + where_3 + ";"
            if cursor:
                cursor.execute(t_4)
                print(cursor.fetchall())
                connection.commit()
